...
int main()
{
	...
	CrearCilindro(10, 1.0f);//índice 5 en MeshList ()
	CreateShaders();
	...
	*/
	...
	glm::mat4 modelCabina(1.0); // Guarda la traslacion de la cabina
	...
	while (!mainWindow.getShouldClose())
	{
		...
		modelaux = model;
		modelCabina = model; // Se guarda la traslacion de la cabina
		...
		// Se hereda la traslacion de la primera caja para hacer las llantas y el soporte
		...
		modelCabina = model; // Lo usamos para heredarlo a las llantas

		//model = glm::rotate(model, glm::radians(30.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::scale(model, glm::vec3(7.0f, 1.5f, 7.0f));

		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));

		color = glm::vec3(0.5f, 0.3f, 1.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		...
		model = glm::scale(model, glm::vec3(1.0f, .7f, 1.0f));
		...
		// 2
		model = modelCabina;
		model = glm::translate(model, glm::vec3(-3.0f, -0.8f, -2.0f));
		model = glm::rotate(model, glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
		model = glm::scale(model, glm::vec3(1.0f, .7f, 1.0f));
